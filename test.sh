...
stress_cpu() {
    local cores=$1
    echo "Stressing CPU: $cores cores for 15 seconds..."
    stress-ng --cpu $cores --timeout 15s
}

# Function to stress both CPU and RAM at the same time
stress_mixed() {
    local cpu_cores=$1
    echo "Stressing CPU and RAM (80%) for 15 seconds..."
    stress-ng --cpu $cpu_cores --vm 1 --vm-bytes 80% --timeout 15s
}
